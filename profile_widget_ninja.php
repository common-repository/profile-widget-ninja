...
Plugin URI: https://wordpress.org/plugins/profile-widget-ninja/
    ...
    class Profile_Widget_Ninja extends WP_Widget {
        ...
        public function form($instance) {
            ...
            <div class="profile-widget-ninja-form">
                ...
                <!-- WordPress link -->
                <p>
                    <label for="<?php echo $this->get_field_id('pwn_wordpress_url'); ?>"><?php _e('WordPress URL:', 'profile-widget-ninja') ?></label>
                    <input type="text" class="widefat" id="<?php echo $this->get_field_id('pwn_wordpress_url'); ?>" name="<?php echo $this->get_field_name('pwn_wordpress_url'); ?>" value="<?php if (isset($instance['pwn_wordpress_url']) && !empty($instance['pwn_wordpress_url'])) { echo esc_attr($instance['pwn_wordpress_url']); } ?>" />
                </p>
        ...
        function show_shortcode_for_widget( $widget_ref, $return, $instance ) {
            if ($widget_ref->id_base !== 'profile_widget_ninja') {
                return;
            }
            ...
            <!-- Donation link -->
            <p>
                <a href="https://www.paypal.me/pankajkumarmondal" target="_blank"><img src="<?php echo plugins_url('icons/paypal.png', __FILE__) ?>" alt="Donate" /> <?php _e('Support Development of this plugin', 'profile-widget-ninja') ?></a>
            </p>
            <?php
        }
        ...
        public function widget($args, $instance) {
            ...
            <div class="pwn-profile-widget-ninja-wrapper pwn-profile-widget-ninja-layout-<?php echo (isset( $instance['pwn_alignment'])) ? $instance['pwn_alignment'] : 'center'; ?> pwn-layout-style-<?php echo (isset( $instance['pwn_layout_style'])) ? $instance['pwn_layout_style'] : 'style1'; ?> pwn-profile-widget-ninja-avatar-<?php echo (isset($instance['pwn_profile_image_shape'])) ? $instance['pwn_profile_image_shape'] : ''; ?>"<?php if (!empty($widget_custom_background_color)) { echo ' style="' . $widget_custom_background_color . '"'; } ?>>
                ...
                if ((isset($instance['pwn_email_id']) && !empty($instance['pwn_email_id'])) ||
                    ...
                    (isset($instance['pwn_wordpress_url']) && !empty($instance['pwn_wordpress_url'])) ||
                ...
                if ($social_media_links_exist === 'y') {
                    ...
                    // WordPress URL
                    if (isset($instance['pwn_wordpress_url']) && !empty($instance['pwn_wordpress_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_wordpress_url']) .'" target="_blank"><span class="pwn-social-icon wordpress-'.$iconset.'" title="WordPress"></span></a>';
                    }
                    // Blogger URL
        ...
        public function update($new_instance, $old_instance) {
            if (is_admin()) {
                ...
                $old_instance['pwn_wordpress_url'] = (isset ($new_instance['pwn_wordpress_url']) && (!empty ($new_instance['pwn_wordpress_url']))) ? strip_tags ($new_instance['pwn_wordpress_url']) : '';
